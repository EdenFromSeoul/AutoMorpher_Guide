...
- その後、**Mesh StudioなどのMesh編集**ツールを使用して、変換後の衣装を追加で調整します。
    - 特に胸など、体型差が大きい部位は、着せる衣装に近い形状になるように調整してください。
...
**変換元アバターと変換先アバターの足の形状が似ているほど**、より安定した結果が得られます。

そのため、変形を行う前に、

**変換元アバター**と**変換先アバター**の足の形状を、できるだけ同じ状態に調整しておくことをおすすめします。
...
- 靴が水たまりのように平らになってしまう場合は、変換元アバターのFoot側にShrinkが適用されていないかご確認ください。
<div class="doc-media-grid doc-media-grid-2">
  <figure>
    <figcaption>悪い例</figcaption>
...
- **「事前調整を行う」**を選択すると、足のサイズを直接調整できます。
    - 変換後の衣装のArmature配下、Footボーンの直上に、靴のスケール調整**用のscaleSupportBoneが**生成されます。
