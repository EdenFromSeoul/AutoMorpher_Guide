...
- 変形前に衣装の形状を調整するかどうかを選択するオプションです。
    - **事前調整をスキップ**（初期設定）：変形前のBone AdjustmentとMeshの追加補正を行いません。
    - **事前調整を行う**：Bone AdjustmentとMeshの追加補正で衣装の形状を手動で調整できます。

---

## 5. 靴の形状を維持

![事前調整と靴の形状オプション画面]({{BASE_PATH}}/media/fa41cad0ef756772.png)

- 以下の2つのオプションから1つを選択します。
    - **靴の形状を維持**（初期設定）：足周辺のMeshの形状を維持します。
    - **足に合わせて変形**：足の形状に合わせて靴を細かく変形します。
- アバターによっては、靴のフィッティングが正常に適用されなかったり、靴の形状が歪んだりする場合があります。
    - 靴の形状が崩れたり、不自然に歪んだりする場合は、**靴の形状を維持**を選択してください。
